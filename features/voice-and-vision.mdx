---
title: "Voice Mode and Image Analysis in TrinaxAI"
sidebarTitle: "Voice & Vision"
description: "TrinaxAI supports speech-to-text input, text-to-speech responses, and local image analysis with qwen2.5-vl — entirely on your device."
---

TrinaxAI runs voice and vision capabilities entirely on your device. Speech recognition uses the browser's Web Speech API, text-to-speech uses browser TTS, and image analysis runs through a local `qwen2.5-vl` model via Ollama. No audio, no images, and no transcripts are ever sent to a cloud API.
...
TrinaxAI can analyse images and screenshots you attach to the chat. The entire analysis runs locally on your machine through a `qwen2.5-vl` vision-language model served by Ollama.
