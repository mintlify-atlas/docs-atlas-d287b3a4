...
| **Memory writes** | `POST /v1/memory`, `DELETE /v1/memory/{id}`, `POST /v1/memory/refresh` |
| **Collection management** | `POST /collections`, `PATCH /collections/{id}`, `DELETE /collections/{id}` |
| **Research** | `POST /v1/research` |
| **Knowledge Browser** | `GET /v1/sources`, `GET /v1/sources/{collection}/{file}/chunks` |
...
Set `TRINAXAI_ADMIN_TOKEN` in `.env` to require a token for all protected endpoints. Pass it in the `Authorization` header:
...
<Note>
  The API reads `Authorization: Bearer <token>` via the standard HTTP header. Some internal code also accepts the legacy `X-Admin-Token` header — use the `Authorization` form for all new integrations.
</Note>
